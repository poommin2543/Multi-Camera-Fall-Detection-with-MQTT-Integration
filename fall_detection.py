...
# Camera Topics for Location Mapping
camera_topics = {
    "Camera 1": "/Location1",
    "Camera 2": "/Location2",
    "Camera 3": "/Location3",
                ...
                if abs(nose_y - hip_y) < 80:  # ถ้าระยะห่างระหว่างจมูกและสะโพกสั้น แสดงว่ากำลังล้ม
                    if fall_timers[index] == 0:
                        ...
                        if not fall_detected[index]:
                            location_message = f"{camera_topics[camera_names[index]]}"
                            print(f"🚨 Fall detected from {camera_names[index]}")
                            mqtt_client.publish(topic_name, f"{location_message}")
